...
inputOp :: Program -> Int -> IO ()
inputOp prog ip = do
  putStr "INPUT: "
  read <$> getLine >>= writeArg prog ip 1
...
outputOp :: Program -> Int -> IO ()
outputOp prog ip = readArg prog ip 1 >>= putStrLn . show
  ...
  where
    go ip = do
      ...
      (`mod` 100) <$> readArray prog ip >>= \case
        ...
        3 -> inputOp prog ip >> go (ip + 2)
        4 -> outputOp prog ip >> go (ip + 2)
