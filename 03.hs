...
parseWire s = parseMove <$> splitOn "," s

trace :: [Move] -> [(Int, Int, Int)]
trace ms = mconcat (go ms 0 0 0)
  where
    go [] _ _ _ = []
    go (m:ms) steps x y = let (points, x', y', s') = expand m steps x y
                          in points : go ms (steps + s') x' y'

    expand (Left d) steps x y = ( (\delta -> (x - delta, y, steps + delta)) <$> [1..d], x - d, y, d)
    expand (Right d) steps x y = ( (\delta -> (x + delta, y, steps + delta)) <$> [1..d], x + d, y, d)
    expand (Down d) steps x y = ( (\delta -> (x, y - delta, steps + delta)) <$> [1..d], x, y - d, d)
    expand (Up d) steps x y = ( (\delta -> (x, y + delta, steps + delta)) <$> [1..d], x, y + d, d)

intersect :: (Ord a, Eq a) => [a] -> [a] -> [a]
intersect [] _ = []
intersect _ [] = []
intersect (a:as) (b:bs)
  | a == b = a : intersect (dropWhile (==a) as) (dropWhile (==a) bs)
  | a < b = intersect as (b:bs)
  | otherwise = intersect (a:as) bs

intersectBy :: (Ord key, Eq key) => (a -> key) -> [a] -> [a] -> [(key, a, a)]
intersectBy _ [] _ = []
intersectBy _ _ [] = []
intersectBy f (a:as) (b:bs)
  | f a == f b = (f a, a, b) : intersectBy f (dropWhile (\x -> f x == f a) as) (dropWhile (\x -> f x == f a) bs)
  | f a < f b = intersectBy f as (b:bs)
  | otherwise = intersectBy f (a:as) bs


main :: IO ()
main = do
  forM_ problems $ \(raw1, raw2) -> do
    let t1 = sort $ trace $ parseWire raw1
        t2 = sort $ trace $ parseWire raw2
        cumDist (_, (_, _, a), (_, _, b)) = a + b
    putStrLn "\n\n=========================\n";
    putStrLn $ show $ sort $ cumDist <$> intersectBy (\(x, y, steps) -> (x, y)) t1 t2
  pure ()
